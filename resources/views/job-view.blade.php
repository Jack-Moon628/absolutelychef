            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="col-md-8 text-white-50">
                        <h1 class="text-danger">{!! $job->job_title !!}</h1>
                        <p >
                        ...
                        <p>
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#applyJobModal" ><i class="la la-calendar-plus-o"></i> @lang('app.apply_online') </button>

                            @if($job->employer->followable)
                                @if(auth()->check() && auth()->user()->isEmployerFollowed($job->employer->id))
                                    <button type="button" class="btn btn-danger employer-follow-button" data-employer-id="{{$job->employer->id}}"><i class="la la-minus-circle"></i> @lang('app.unfollow') {{$employer->company}} </button>
                                @else
                                    <button type="button" class="btn btn-outline-primary employer-follow-button" data-employer-id="{{$job->employer->id}}"><i class="la la-plus-circle"></i> @lang('app.follow') {{$employer->company}} </button>
                                @endif
                            @endif
                    ...
                    <div class="col-md-4">
                        <div class="job-view-lead-position-box">
                            <p class="text-white-50"><strong>@lang('app.about_position')</strong></p>
                            <h5>{{$job->position}} <span class="text-white-50 text-small">(@lang('app.'.$job->job_type))</span></h5>
                            <p class="m-0">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="job-view-container shadow rounded bg-white p-4 mb-4 ">
                        ...
                        <h4 class="text-danger">{!! $job->job_title !!}</h4>
                        <hr class="m-0">
                        <p class="text-muted">
                            <i class="la la-briefcase"></i> @lang('app.'.$job->job_type)
                ...
                <div class="col-md-4">
                    ...
                    @if($job->is_premium)
                        <div class="widget-box bg-white p-3 mb-3 shadow rounded">
                            ...
                            <p class="m-0 text-white-50">From {{$employer->company}}</p>
                    ...
                    <div class="widget-box bg-white p-3 mb-3 shadow rounded">
                        ...
                        <p>
                            @if($job->gender == 'any')
                                ...
                                <span class="text-white-50">(@lang('app.'.$job->gender))</span>
                            @else
                        ...
                        <p>
                            <i class="la la-calendar-times-o"></i> @lang('app.deadline') : {{$job->deadline->format(get_option('date_format'))}} <span class="text-small text-white-50">{{$job->deadline->diffForHumans()}}</span>
                        </p>
                        ...
                        <p>
                            @if($job->employer->followable)
                                @if(auth()->check() && auth()->user()->isEmployerFollowed($job->employer->id))
                                    <button type="button" class="btn btn-danger employer-follow-button" data-employer-id="{{$job->employer->id}}"><i class="la la-minus-circle"></i> @lang('app.unfollow') {{$employer->company}} </button>
                                @else
                                    <button type="button" class="btn btn-danger employer-follow-button" data-employer-id="{{$job->employer->id}}"><i class="la la-plus-circle"></i> @lang('app.follow') {{$employer->company}} </button>
                                @endif
                            @endif
                        </p>
                    ...
                    <div class="widget-box bg-white p-3 shadow rounded">
                        ...
                        <div class="additional-job-action-box">
                            ...
                            @if($job->employer && $job->employer->company_slug)
                    ...
                    <form action="{{route('apply_job')}}" method="post" id="applyJob" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group {{ $errors->has('resume')? 'has-error':'' }}">
                                <label for="resume" class="control-label">@lang('app.cv'):</label>
                                <input type="file" class="form-control {{e_form_invalid_class('resume', $errors)}}" id="resume" name="resume">
                                <p class="text-white-50">@lang('app.resume_file_types')</p>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{route('flag_job_post', $job->id)}}" method="post">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">@lang('app.close')</button>
                            <button type="submit" class="btn btn-danger"><i class="la la-flag-o"></i> @lang('app.flag_this_job')</button>
                        </div>
                    </form>
